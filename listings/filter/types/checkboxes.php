...
if ( stm_is_multilisting() ) {
	$listing_type = STMMultiListing::stm_get_current_listing_slug();
	if ( ! empty( $listing_type ) && stm_listings_post_type() !== $listing_type ) {
		$checkboxes = stm_get_car_filter_checkboxes( $listing_type );
	}
}
...
if ( ! empty( $checkboxes ) ) {
	foreach ( $checkboxes as $checkbox ) {
		...
		<div
			...
			<a class="title <?php echo ( 'false' === esc_attr( $listing_rows_numbers_default_expanded ) ) ? 'collapsed' : ''; ?> " data-toggle="collapse" href="#accordion-<?php echo esc_attr( $checkbox['slug'] ); ?>"
			...
			<div class="stm-accordion-content">
				<div class="collapse content <?php echo ( 'true' === esc_attr( $listing_rows_numbers_default_expanded ) ) ? 'in' : ''; ?>" id="accordion-<?php echo esc_attr( $checkbox['slug'] ); ?>">
								...
								foreach ( $terms as $rest_term ) {
									...
									if ( empty( $image ) ) :
										...
										<input type="checkbox" name="<?php echo esc_attr( $checkbox['slug'] ); ?>[]"
											...
											<?php echo ( in_array( $rest_term->slug, $selected_options ) ) ? 'checked' : '', true; ?>
										/>
