				...
				<div class="stm-user-private-sidebar">
					...
					<div class="clearfix stm-user-top">
						...
						<div class="stm-user-profile-information">
							<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'settings' ), stm_get_author_link( '' ) ) ); ?>"
						...
						<div class="stm-user-avatar">
							<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'settings' ), stm_get_author_link( '' ) ) ); ?>">
								<?php if ( ! empty( $user_fields['logo'] ) ) : ?>
									<img class="img-responsive img-avatar"
											src="<?php echo esc_url( $user_fields['logo'] ); ?>"/>
								<?php else : ?>
									<img class="img-responsive img-avatar"
											src="<?php stm_get_dealer_logo_placeholder(); ?>"/>
								<?php endif; ?>
							</a>
						</div>
					...
					<?php if ( stm_pricing_enabled() ) : ?>
						<?php $stm_user_active_subscriptions = stm_user_active_subscriptions( false, $user->ID ); ?>
						<div class="stm-user-current-plan-info heading-font">
							<?php if ( ! empty( $stm_user_active_subscriptions ) ) : ?>
								...
								<div class="stm-plan-name"><?php echo esc_html( stm_do_lmth( $stm_user_active_subscriptions['plan_name'] ) ); ?></div>
								<?php
								$day_left     = false;
								...
								<script type="text/javascript">
									jQuery(document).ready(function () {
										var $ = jQuery;
										$(".stm-start-countdown")
											.countdown("<?php echo esc_html( stm_do_lmth( $stm_user_active_subscriptions['expires'] ) ); ?>", function (event) {
												$(this).text(
							...
							<?php elseif ( $date_expires ) : ?>
								<div class="sub-title"><?php esc_html_e( 'Subscription renewal', 'motors' ); ?></div>
								<div class="days-left"><?php echo esc_html( date( 'm.d.Y', $date_expires ) ); ?></div>
							<?php endif; ?>
					...
					<?php else : ?>
						<div class="show-my-profile">
							<a href="<?php echo esc_url( stm_get_author_link( 'myself-view' ) ); ?>" target="_blank"><i
										class="fas fa-external-link-alt"></i><?php esc_html_e( 'Show my Public Profile', 'motors' ); ?>
							</a>
						</div>
					<?php endif; ?>
